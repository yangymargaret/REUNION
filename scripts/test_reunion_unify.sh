...
PATH1=$input_dir
atac_data=$PATH1/atac_pbmc.h5ad  # file path of single cell ATAC-seq AnnData is optional if ATAC-seq metacell data are provided
rna_data=$PATH1/rna_pbmc.h5ad	 # file path of single cell RNA-seq AnnData is optional if RNA-seq metacell data are provided
...
peak_bg_num=100
file_bg=$PATH1/test_peak_read.$data_file_type.normalize.bg.$peak_bg_num.1.csv
...
for i1 in {0..0}; do
	run_id1=$i1
	python test_unify_compute_group_1_copy1_1.py --run_id $run_id1 \
											-b $b1 \
